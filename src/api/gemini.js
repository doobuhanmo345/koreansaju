...
export const fetchGeminiAnalysis = async (prompt) => {
  try {
    // 1. 모델 목록 조회 및 자동 선택
    const modelListRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${API_KEY}`);
    const modelData = await modelListRes.json();
    
    // 1.5-flash -> pro 순으로 찾기
    let targetModel = modelData.models?.find(m => m.name.includes("1.5-flash"))
